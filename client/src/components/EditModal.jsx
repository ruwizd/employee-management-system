...
export default function EditModal({ fetchData, row }) {
    ...
    const uploadForm = async (prediction) => {
        ...
        try {
            // const docRef = await db.collection("employee").doc(form.id).update({
            //     ...form,
            //     prediction,
            //     last_prediction: moment().format("DD-MM-YYYY")
            // });
            const docRef = doc(collection(db, "employee"), form.id);
    ...
    return (
        <>
            <button onClick={() => setShowEditModal(true)} className="font-medium text-blue-600 dark:text-blue-500 hover:underline">Update </button>
            {showEditModal ? (
                <>
                    <div
